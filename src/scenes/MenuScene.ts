...
export class ScientiaMenuScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const level2Unlocked = progressSystem.isLevelUnlocked('level2')
    ...
    const level2Button = new UIButton(
      this,
      w / 2,
      365,
      300,
      46,
      level2Unlocked ? 'Level 2: Lost Perfume Formula' : 'Level 2: Lost Perfume Formula [LOCKED]',
      0x4a3020,
      0x6b462f,
      () => {
        if (!level2Unlocked) {
    ...
    level2Button.enabled = level2Unlocked
    ...
    const level3Button = new UIButton(
      ...
      300,
      46,
      level3Unlocked ? 'Level 3: Invisible Energy' : 'Level 3: Invisible Energy [LOCKED]',
      0x123225,
      0x1f6349,
      () => {
        if (!level3Unlocked) {
    ...
    if (!level3Unlocked) {
      this.add.text(w / 2, 486, 'Complete Level 2 to unlock this mission.', {
        fontSize: '12px',
        color: '#8a7f6e',
        fontFamily: 'Georgia, serif',
      }).setOrigin(0.5)
    ...
    for (let i = 0; i < dotMarkers.length; i++) {
      ...
      const playable = i === 0 || (i === 2 && level2Unlocked) || (i === 3 && level3Unlocked)
