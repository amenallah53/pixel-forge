...
export class Level4CompleteScene extends Phaser.Scene {
  constructor() {
    super({ key: 'Level4CompleteScene' })
  }
  ...
  create(): void {
    ...
    this.add.text(w / 2, 68, LEVEL4.fragment.name, {
      fontSize: '24px',
      color: '#55d6ff',
      fontFamily: 'Georgia, serif',
      fontStyle: 'italic',
  ...
  private createFragment(w: number, h: number): void {
    ...
    const size = 58
    fragment.fillStyle(LEVEL4.fragment.color, 0.86)
    fragment.beginPath()
    for (let i = 0; i < 6; i++) {
