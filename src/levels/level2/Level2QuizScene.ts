...
export class Level2QuizScene extends Phaser.Scene {
  private quizSystem!: QuizSystem
  private progressSystem!: ProgressSystem

  constructor() {
    super({ key: 'Level2QuizScene' })
  }
  ...
  private async startQuiz(): Promise<void> {
    ...
    results.forEach((r) => {
      if (r.correct) this.progressSystem.addXP('level2', 25)
    })

    const config = LEVELS.level2
    this.progressSystem.addFragment('level2', config.fragment)
    this.progressSystem.completeLevel('level2', score)
