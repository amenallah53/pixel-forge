...
export class MirrorLabyrinthScene extends Phaser.Scene {
  ...
  private createRoom(w: number, h: number): void {
    ...
    this.add.graphics().setDepth(2).lineStyle(2, 0x2a2a4a, 0.3).strokeRect(15, 15, w - 30, h - 30)
  }
  ...
  private createEmitter(w: number, h: number): void {
    ...
    g.fillStyle(0xffff88, 0.2).fillCircle(this.lightSource.x, this.lightSource.y, 18)
    g.fillStyle(0xffffff, 0.06).fillCircle(this.lightSource.x, this.lightSource.y, 30)

    const glow = this.add.graphics().setDepth(13)
    glow.fillStyle(0xffff44, 0.04).fillCircle(this.lightSource.x, this.lightSource.y, 40)
    this.tweens.add({ targets: glow, alpha: 0.5, duration: 1000, yoyo: true, repeat: -1 })

    this.tweens.add({ targets: g, alpha: { from: 0.8, to: 1 }, duration: 600, yoyo: true, repeat: -1 })

    this.add.text(this.lightSource.x, this.lightSource.y - 28, 'LIGHT SOURCE', {
      fontSize: '9px', color: '#ffff44', fontFamily: 'Georgia, serif',
    }).setOrigin(0.5).setDepth(15)

    const arrow = this.add.graphics().setDepth(13)
    arrow.fillStyle(0xffff44, 0.2)
    arrow.fillTriangle(70, 345, 70, 355, 95, 350)
    this.tweens.add({ targets: arrow, alpha: { from: 0.3, to: 0.7 }, duration: 500, yoyo: true, repeat: -1 })
  }

  private createTarget(w: number, h: number): void {
    const tx = this.targetPos.x; const ty = this.targetPos.y
    ...
    this.tweens.add({ targets: g, scaleX: { from: 0.95, to: 1.05 }, scaleY: { from: 0.95, to: 1.05 }, duration: 1500, yoyo: true, repeat: -1 })

    this.add.text(tx, ty + 28, 'TARGET', {
      fontSize: '9px', color: '#88ccff', fontFamily: 'Georgia, serif',
    }).setOrigin(0.5).setDepth(15)
  }
  ...
  private createMirrors(): void {
    ...
    for (const cfg of configs) {
      ...
      const label = this.add.text(0, 58, cfg.label + ' [0]', {
        fontSize: '10px', color: '#88a4ff', fontFamily: 'Georgia, serif',
      }).setOrigin(0.5).setDepth(15)
      container.add(label)
  ...
  private showClickFlash(x: number, y: number): void {
    ...
    this.tweens.add({ targets: flash, alpha: 0, scale: 1.5, duration: 350, onComplete: () => flash.destroy() })
  }
  ...
  private createUI(w: number, h: number): void {
    ...
    this.add.text(w / 2, 18, 'STEP 3: REFLECTION', {
      fontSize: '11px', color: '#ffd700', fontFamily: 'Georgia, serif', fontStyle: 'italic',
    }).setOrigin(0.5).setDepth(29)

    this.hintText = this.add.text(w / 2, 36, 'Goal: bounce the beam into the crystal. Click a mirror to rotate it by 30 deg.', {
      fontSize: '9px', color: '#b6b6d9', fontFamily: 'Georgia, serif',
    }).setOrigin(0.5).setDepth(29)
    ...
    this.angleText = this.add.text(w / 2, h - 15, '', {
      fontSize: '10px', color: '#5588aa', fontFamily: 'Georgia, serif',
  ...
  private updateBeam(): void {
    ...
    if (result.segments.length < 2) {
      ...
      this.hintText.setText('Beam does not hit any mirror! Position mirrors in its path.')
      return
    }
  ...
  private traceBeam(): { segments: { x: number; y: number }[]; hitTarget: boolean } {
    const segs: { x: number; y: number }[] = [{ x: this.lightSource.x, y: this.lightSource.y }]
    let angle = 0
    ...
    for (let bounce = 0; bounce < 5; bounce++) {
      let bestHit: { x: number; y: number; dist: number; idx: number } | null = null
      ...
      this.showAngles(cx, cy, angle, mirror.rotation)
    }
  ...
  private rayMirrorHit(
    ox: number, oy: number, angle: number, mirror: MirrorObj,
  ...
  private drawBeam(segs: { x: number; y: number }[]): void {
    ...
    for (let i = 0; i < segs.length - 1; i++) {
      const s = segs[i]; const e = segs[i + 1]

      this.glowG.lineStyle(12, 0xffff44, 0.06 * pulse)
      this.glowG.beginPath(); this.glowG.moveTo(s.x, s.y); this.glowG.lineTo(e.x, e.y); this.glowG.strokePath()

      this.glowG.lineStyle(6, 0xffff44, 0.12 * pulse)
      this.glowG.beginPath(); this.glowG.moveTo(s.x, s.y); this.glowG.lineTo(e.x, e.y); this.glowG.strokePath()

      this.beamG.lineStyle(2.5, 0xffff44, 0.85 * pulse)
      this.beamG.beginPath(); this.beamG.moveTo(s.x, s.y); this.beamG.lineTo(e.x, e.y); this.beamG.strokePath()
    ...
    if (segs.length >= 2 && !this.solved) {
      ...
      if (dist < 30) {
        ...
        this.beamG.fillStyle(0x44ffaa, 0.3 * pulse).fillCircle(last.x, last.y, 18)
      } else if (dist < 100) {
    ...
    const em = this.add.particles(lastSeg.x, lastSeg.y, 'particle', {
      speed: { min: 3, max: 10 },
      angle: { min: 0, max: 360 },
  ...
  private drawGhostPreview(prev: { x: number; y: number }, last: { x: number; y: number }): void {
    const dx = last.x - prev.x; const dy = last.y - prev.y
    const len = Math.sqrt(dx * dx + dy * dy)
    if (len < 1) return
    const nx = dx / len; const ny = dy / len
    const glx = last.x + nx * 60; const gly = last.y + ny * 60
    ...
    for (let t = 0; t < 1; t += 0.08) {
      ...
      this.ghostG.beginPath(); this.ghostG.moveTo(sx, sy); this.ghostG.lineTo(ex, ey); this.ghostG.strokePath()
    }
  }

  private showAngles(cx: number, cy: number, outAngle: number, _mirrorAngle: number): void {
    const normal = _mirrorAngle
    const inc = Phaser.Math.RadToDeg(Math.abs(outAngle - normal)) % 180
    const ref = 180 - inc
    this.angleText.setText(`Incoming = ${Math.round(Math.min(inc, 180 - inc))}deg  Reflected = ${Math.round(ref >= 90 ? 180 - ref : ref)}deg`)
    this.angleText.setAlpha(1)
  }

  private distToTarget(x: number, y: number): number {
    return Phaser.Math.Distance.Between(x, y, this.targetPos.x, this.targetPos.y)
  }
  ...
  private onPuzzleSolved(): void {
    ...
    for (let i = 0; i < result.segments.length - 1; i++) {
      const s = result.segments[i]; const e = result.segments[i + 1]
    ...
    const burst = this.add.particles(this.targetPos.x, this.targetPos.y, 'particle', {
      speed: { min: 40, max: 150 }, angle: { min: 0, max: 360 },
      scale: { start: 0.7, end: 0 }, alpha: { start: 0.9, end: 0 },
      lifespan: 2000, quantity: 30, tint: 0x44ffaa, emitting: false,
    }).setDepth(25)
    burst.explode(30)
    this.time.delayedCall(400, () => burst.explode(20))
    this.time.delayedCall(800, () => {
      const b2 = this.add.particles(this.targetPos.x, this.targetPos.y, 'particle', {
        speed: { min: 10, max: 30 }, angle: { min: 0, max: 360 },
        scale: { start: 0.4, end: 0 }, alpha: { start: 0.6, end: 0 },
        lifespan: 3000, quantity: 10, tint: 0x88ddff, emitting: false,
      }).setDepth(25)
      b2.explode(15)
    })

    const overlay = this.add.graphics().setDepth(28)
    overlay.fillStyle(0x000000, 0).fillRect(0, 0, this.scale.width, this.scale.height)
    this.tweens.add({ targets: overlay, alpha: 0.35, duration: 600 })

    const txt = this.add.text(this.scale.width / 2, this.scale.height / 2 - 10, 'REFLECTION CAPTURED', {
      fontSize: '22px', color: '#44ffaa', fontFamily: 'Georgia, serif', fontStyle: 'italic',
    }).setOrigin(0.5).setDepth(30).setAlpha(0)
    this.tweens.add({ targets: txt, alpha: 1, scale: { from: 0.5, to: 1 }, duration: 500, ease: 'Back.easeOut' })

    const peda = this.add.text(this.scale.width / 2, this.scale.height / 2 + 30, 'Angle of incidence = Angle of reflection', {
      fontSize: '12px', color: '#88ddff', fontFamily: 'Georgia, serif', fontStyle: 'italic',
    }).setOrigin(0.5).setDepth(30).setAlpha(0)
  ...
  private async startIntroDialogue(): Promise<void> {
    ...
    this.hintText.setText('Click the mirrors one at a time. Watch the beam path and guide it into the crystal.')
    this.time.delayedCall(3000, () => {
      if (!this.solved) this.hintText.setText('Tip: small rotations change the beam a lot. Start with the mirror closest to the light.')
    })
    this.updateBeam()
  }
